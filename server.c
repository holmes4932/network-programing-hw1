...
void upload_file(char *buffer,int fd,int ret){
	...
	if(access("upload/", 0) == -1){
		mkdir("upload/",0777);
	}

	int upload_fd = open(path,O_CREAT|O_WRONLY|O_TRUNC|O_SYNC,S_IRWXO|S_IRWXU|S_IRWXG);
	if (t != NULL){
		write(upload_fd,s+1,filesize);
	}
	else{
		char buf[BUFSIZE+1];
		int start = (int )(s - &buffer[0])+1;
        int last_write = write(upload_fd,s+1,ret -start -61);
        int last_ret = ret;
        memcpy (buf,s+1+last_write,61);
        int total=0;

        total += ret;
        while ((ret=read(fd, buffer,BUFSIZE))>0){
            total += ret;
            write(upload_fd,buf,61);
            last_write = write(upload_fd,buffer,ret - 61);
            memcpy (buf,buffer+last_write,61);
            last_ret = ret;
            if (total>=filesize-1){
                break;
            }
